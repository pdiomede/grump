...
DASHBOARD_URL = os.getenv('DASHBOARD_URL', 'http://localhost:38081/')
...
# Use production paths if they exist, otherwise use local paths
if PRODUCTION_CONFIG_DIR.exists():
    ...
    WEB_DIR = SCRIPT_DIR
    print(f"🔧 Running in DEVELOPMENT mode from: {SCRIPT_DIR}")
    print(f"   For production, deploy to: {PRODUCTION_CONFIG_DIR}")
    print()
...
def load_whitelist():
    ...
    whitelist_file = CONFIG_DIR / 'allowed_people.txt'
...
<head>
    <style>
        body {{ font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background: linear-gradient(135deg, #FFD4B2 0%, #FFA07A 50%, #FFB88C 100%); margin: 0; padding: 20px; }}
        .container {{ max-width: 600px; margin: 0 auto; background-color: #ffffff; padding: 30px; border-radius: 10px; box-shadow: 0 4px 15px rgba(255, 140, 66, 0.3); }}
        .header {{ text-align: center; margin-bottom: 30px; }}
        .logo {{ font-size: 24px; font-weight: bold; color: #CC5500; }}
        .otp-code {{ background-color: #FFE8D5; border: 2px solid #FF8C42; border-radius: 8px; padding: 20px; text-align: center; margin: 20px 0; }}
        .code {{ font-size: 32px; font-weight: bold; color: #CC5500; letter-spacing: 5px; }}
        .info {{ color: #8B4513; font-size: 14px; margin: 20px 0; }}
        .footer {{ color: #A0826D; font-size: 12px; margin-top: 30px; padding-top: 20px; border-top: 1px solid #FFA07A; text-align: center; }}
    </style>
</head>
<body>
    <div class="container">
...
def index():
    ...
    if email:
        ...
        return static_file('login.html', root=str(WEB_DIR))
...
def serve_assets(filename):
    ...
    return static_file(filename, root=str(WEB_DIR))
...
if __name__ == '__main__':
    ...
    if not index_path.exists():
        print(f"⚠️  WARNING: index.html not found in {WEB_DIR}!")
    ...
    if not login_path.exists():
        print(f"⚠️  WARNING: login.html not found in {WEB_DIR}!")
        print("   Login page is missing - authentication will not work")
        print()
    
    print("🚀 Starting server...")
    print(f"   URL: http://localhost:38081")
    ...
    app.run(host='0.0.0.0', port=38081, debug=False, reloader=False)
